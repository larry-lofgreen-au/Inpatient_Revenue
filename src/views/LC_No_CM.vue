<template>
    <main class="add-lc-view">
        <div class="header-row">
            <h3 class="fw-bold">LCs Waiting for CM</h3>
        </div>
        <hr>
                ...
                <div class="row">
                    <div class="col-lg-4">
                        ...
                        <input type="text" class="form-control" id="facility" readonly
                            v-model="this.$store.state.lc_no_cm_data[this.selectedIndex].facility">
                    </div>
                    <div class="col-lg-4">
                        <label for="caseAccountNumber">Case / Account Number</label>
                        <input type="text" class="form-control" id="caseAccountNumber" readonly
                            v-model="this.$store.state.lc_no_cm_data[this.selectedIndex].caseAccountNumber">
                    </div>
                    <div class="col-lg-4">
                        <label for="dischargeDate">Discharge Date:</label>
                        <input type="text" class="form-control" id="dischargeDate" readonly
                            v-model="this.$store.state.lc_no_cm_data[this.selectedIndex].dischargeDate">
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-4">
            ...
            <form id="lcsForm">
                <table class="table table-striped table-light table-bordered">
                    ...
                    <tbody style="vertical-align: middle;">
                        <tr v-for="(f,index) in this.$store.state.lc_no_cm_data"
                                :value="f"
                                :key="index">
                            <td>{{ f.facility }}</td>
    ...
    export default {
        ...
        computed: {
            GetInstructions() {
                ...
                if(this.$store.state.lc_no_cm_data.length == 0) {
        ...
        methods: {
            AttachCM(index) {
                ...
                this.cm_Amount = this.$store.state.lc_no_cm_data[index].cm_Amount;
                this.lc_hospitalToCollect = this.$store.state.lc_no_cm_data[index].hospitalToCollect;
                this.cm_fileUri = this.$store.state.lc_no_cm_data[index].cm_fileUri;
                this.successMessage = "";
                ...
                fetch(url, options)
                    ...
                    .then((responseData) => {
                        ...
                        if (this.cm_Amount != this.lc_hospitalToCollect) {
                            ...
                            this.successMessage += this.$store.state.lc_no_cm_data[this.selectedIndex].facility
                ...
                fetch(url, options)
                    ...
                    .then((responseData) => {
                        ...
                        for(const item of responseData.lC_List)
                        {
                            ...
                            this.$store.state.lc_no_cm_data.push(newItem);
